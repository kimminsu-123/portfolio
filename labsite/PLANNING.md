...
**`Lectures` 탭**
| id | title | week | fileType(ppt/html) | sourceFileId | slidesFileId(ppt 전용, 변환된 Slides 사본) | slideImageUrls(ppt 전용, JSON 배열) | uploadedAt |
...
- **Slides 썸네일 이미지 URL의 유효기간**: `presentations.pages.getThumbnail`이 반환하는 이미지 URL이 서명된(signed) 임시 URL이라 일정 시간 후 만료될 가능성이 있습니다. 만료된다면 업로드 시점에 한 번 캐싱해두는 설계(§4-3)가 몇 주 뒤엔 깨진 이미지로 보일 수 있어, 실제 만료 여부·기간을 반드시 확인해야 합니다. 만료된다면 "캐싱"이 아니라 "슬라이드 파일 ID만 저장해두고 조회 시마다 재생성" 방식으로 바꿔야 합니다(쿼터 비용은 늘지만 안정성 확보).
- **pptx → Google Slides 변환의 정확한 API 절차**: Drive API로 업로드할 때 `mimeType`을 Slides 포맷으로 지정해 변환하는 방식으로 알고 있으나, Apps Script의 `Drive` 고급 서비스에서 정확히 어떤 파라미터로 호출해야 하는지는 실제 구현 시 문서 재확인 필요.
...
- [ ] Google Drive에 "강의자료" 폴더, "과제 제출 루트" 폴더 각각 생성 후 폴더 ID 확보 → Script Properties의 `LECTURE_FOLDER_ID`, `SUBMISSION_ROOT_FOLDER_ID`에 입력 (아직 안 함)
...
2. **강의자료 목록/미리보기 접근 범위**: 요구사항 1(외부 공개)을 감안하면 "미리보기는 로그인 없이 아무나 가능, 다운로드만 관리자 게이트"로 가정하고 설계했는데 맞을까요, 아니면 미리보기도 로그인(수강생만) 해야 볼 수 있게 할까요?
...
4. 강의자료 목록 조회 + 미리보기 (읽기 전용 기능부터) ← **다음 단계**
5. 관리자 업로드/삭제 (쓰기 + 인증 게이트)
6. 과제 생성/삭제 (관리자)
